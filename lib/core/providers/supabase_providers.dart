    ...
    Supabase.instance.client.auth.onAuthStateChange.listen((data) {
      ...
      if (user != null) {
        ...
        final appUser = AppUser(
          ...
          subscriptionPlan: user.userMetadata?['subscription_plan'] ?? 'free',
          isActive: true,
...
// Finance Service Provider (removed - using TransactionService only)
...
// ============= RESTAURANT-AWARE DATA PROVIDERS =============
// These providers automatically refresh data when restaurant selection changes

/// Restaurant-aware menu items provider
final restaurantMenuItemsProvider = FutureProvider<List<dynamic>>((ref) async {
  final currentRestaurant = ref.watch(currentRestaurantProvider);
  if (currentRestaurant == null) return [];

  final menuService = ref.watch(supabaseMenuServiceProvider);
  // Pass the restaurant ID to get only that restaurant's menu items
  return await menuService.getMenuItems(restaurantId: currentRestaurant.id);
});

/// Restaurant-aware orders provider
final restaurantOrdersProvider = FutureProvider<List<dynamic>>((ref) async {
  final currentRestaurant = ref.watch(currentRestaurantProvider);
  if (currentRestaurant == null) return [];

  final orderService = ref.watch(supabaseOrderServiceProvider);
  // Pass the restaurant ID to get only that restaurant's orders
  return await orderService.getOrders(restaurantId: currentRestaurant.id);
});

/// Restaurant-aware customers provider
final restaurantCustomersProvider = FutureProvider<List<dynamic>>((ref) async {
  final currentRestaurant = ref.watch(currentRestaurantProvider);
  if (currentRestaurant == null) return [];

  final customerService = ref.watch(supabaseCustomerServiceProvider);
  // Pass the restaurant ID to get only that restaurant's customers
  return await customerService.getCustomers(restaurantId: currentRestaurant.id);
});

/// Restaurant-aware menu categories provider
final restaurantMenuCategoriesProvider = FutureProvider<List<dynamic>>((ref) async {
  final currentRestaurant = ref.watch(currentRestaurantProvider);
  if (currentRestaurant == null) return [];

  final menuService = ref.watch(supabaseMenuServiceProvider);
  // Get menu categories for the current restaurant
  return await menuService.getCategories(restaurantId: currentRestaurant.id);
});

/// Provider to invalidate data when restaurant changes
final restaurantDataInvalidatorProvider = Provider<void>((ref) {
  final currentRestaurant = ref.watch(currentRestaurantProvider);

  // Listen to restaurant changes and invalidate data immediately
  ref.listen<Restaurant?>(currentRestaurantProvider, (previous, next) {
    if (previous != next && next != null) {
      // Invalidate all restaurant-specific data when restaurant changes
      ref.invalidate(restaurantMenuItemsProvider);
      ref.invalidate(restaurantOrdersProvider);
      ref.invalidate(restaurantCustomersProvider);
      ref.invalidate(restaurantMenuCategoriesProvider);
    }
  });
