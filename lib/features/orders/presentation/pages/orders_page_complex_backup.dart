...
class _OrdersPageState extends ConsumerState<OrdersPage> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
    ...
    try {
      ...
      setState(() {
        ...
        for (int i = 0; i < _orders.length; i++) {
          ...
          final hasSyncingItems = currentOrder.items.any((item) => item.isLocalPending);
        ...
        case SyncEventType.itemCompletionUpdated:
          ...
          if (mounted) {
            ...
            final hasSyncingItems = _orders.any((order) =>
              order.items.any((item) => item.isLocalPending));
      ...
      if (orderIndex != -1) {
        ...
        if (itemIndex != -1) {
          ...
          updatedItems[itemIndex] = item.copyWith(
            ...
            isLocalPending: true, // Show syncing indicator
          ...
          debugPrint('✅ [TOGGLE COMPLETION] UI updated with isLocalPending: true - timer started');
        }
      }
    });
          ...
          if (orderIndex != -1) {
            ...
            if (itemIndex != -1) {
              ...
              updatedItems[itemIndex] = updatedItems[itemIndex].copyWith(
                isLocalPending: false, // Clear syncing indicator
              ...
              if (syncStartTime != null) {
                ...
                debugPrint('🎯 [TOGGLE COMPLETION] ${beforeItem.menuItemName} isLocalPending: ${beforeItem.isLocalPending} → FALSE');
  ...
  Widget _buildCompletionButton(OrderItem item) {
    ...
    final isSyncing = item.isLocalPending;
    final hasError = item.syncError != null;

    // Debug log the current state being rendered
    if (isSyncing) {
      debugPrint('🎨 [UI RENDER] ${item.menuItemName} showing as SYNCING (isLocalPending: $isSyncing, isCompleted: $isCompleted)');
    }
